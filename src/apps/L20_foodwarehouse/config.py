# Configuration loading for the deterministic L20 foodwarehouse workflow.
...
# Build stable repository-relative paths for the L20 app.
def build_app_paths() -> AppPaths:
    ...
    return AppPaths(
        ...
        data_dir=data_dir,
        input_file=data_dir / "food4cities.json",
...
def build_safe_config_summary(config: AppConfig) -> dict[str, object]:
    return {
        "app": APP_NAME,
        "data_file": str(config.paths.input_file.relative_to(config.paths.repo_root)),
